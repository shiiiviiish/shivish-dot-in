...
export default function Gallery() {
  ...
  const [activeIndex, setActiveIndex] = useState(0) // kept for lightbox compat
  ...
  return (
    <div style={{background:'#07100d',color:'#e8e4d9',minHeight:'100vh',cursor:isMobile?'auto':'none',fontFamily:"'DM Sans',sans-serif"}}>
      ...
      {/* SEMI-CIRCLE FAN */}
      <div style={{position:'relative',width:'100%',height:'100vh',overflow:'hidden',background:'#07100d',display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
        ...
        {/* Top label */}
        <div style={{position:'absolute',top:90,left:'50%',transform:'translateX(-50%)',zIndex:20,textAlign:'center'}}>
          <p style={{fontSize:11,letterSpacing:'0.2em',textTransform:'uppercase',color:'rgba(232,228,217,0.3)',fontFamily:"'DM Sans',sans-serif",marginBottom:8}}>Gallery · Photography & Editing</p>
          <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:'clamp(32px,5vw,56px)',fontWeight:400,lineHeight:0.95,letterSpacing:'-0.03em',color:'#e8e4d9'}}>
            Browse <em style={{color:'rgba(232,228,217,0.25)',fontStyle:'italic'}}>the shots.</em>
          </h2>
        </div>

        {/* Fan cards */}
        <div id="tour-gallery-grid" style={{position:'relative',width:'100%',height:'100%'}}>
          {photos.map((photo,i)=>{
            const total = photos.length
            const spread = isMobile ? 70 : 90
            const angle = -spread/2 + (spread/(total-1))*i
            const radius = isMobile ? 320 : 460
            const cx = 50 // percent
            const rad = (angle - 90) * Math.PI / 180
            const xPct = cx + (radius * Math.cos(rad) / (isMobile ? 3.6 : 14)) 
            const cardW = isMobile ? 100 : 150
            ...
            return (
              <div
                ...
                onMouseEnter={e=>{
                  e.currentTarget.style.transform=`rotate(${angle}deg) scale(1.12) translateY(-24px)`
                  e.currentTarget.style.boxShadow=`0 24px 56px rgba(0,0,0,0.7), 0 0 24px ${photo.accent}30`
                ...
                style={{
                  ...
                  cursor:'pointer',
                  zIndex: i+1,
                  transition:'transform 0.4s cubic-bezier(0.16,1,0.3,1), box-shadow 0.3s, z-index 0s',
                  // Position in arc
                  left:`calc(50% + ${Math.cos(rad) * radius}px - ${cardW/2}px)`,
                  bottom: `calc(${-radius}px + ${Math.abs(Math.sin(rad)) * radius * 0.3}px - 40px)`,
                  overflow:'hidden',
                }}>
                <div style={{position:'absolute',inset:0,background:`radial-gradient(circle at 40% 40%, ${photo.accent}22 0%, transparent 65%)`}}/>
                <div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:8,padding:12}}>
                  <div style={{width:18,height:18,borderRadius:'50%',background:photo.accent,opacity:0.45}}/>
                  <p style={{fontFamily:"'Instrument Serif',serif",fontSize:isMobile?12:15,color:'rgba(232,228,217,0.75)',fontStyle:'italic',textAlign:'center'}}>{photo.title}</p>
                  <p style={{fontSize:8,color:'rgba(232,228,217,0.3)',letterSpacing:'0.12em',textTransform:'uppercase'}}>{photo.category}</p>
                </div>
              </div>
            )
          })}
        ...
        {/* Bottom hint */}
        <div style={{position:'absolute',bottom:24,left:'50%',transform:'translateX(-50%)',zIndex:20,display:'flex',flexDirection:'column',alignItems:'center',gap:6,fontSize:9,letterSpacing:'0.2em',color:'rgba(232,228,217,0.2)',textTransform:'uppercase',fontFamily:"'DM Sans',sans-serif"}}>
          <span>Hover a card · Scroll for grid</span>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:isMobile?'60px 20px 60px':'100px 48px 80px'}}>
        ...
        {/* Grid */}
        <div id="tour-gallery-grid" style={{display:'grid',gridTemplateColumns:`repeat(${cols},1fr)`,gap:isMobile?8:12}}>
